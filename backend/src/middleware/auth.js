...
// Protect user routes - require user authentication
...
// Protect artisan routes - require artisan authentication
...
const artisanVerified = (req, res, next) => {
  if (req.artisan && req.artisan.isVerified) {
    next();
  } else {
    return res.status(403).json({
      success: false,
      message: 'Artisan account not verified'
    });
  }
};
